...
import static com.shatteredpixel.citnutpixeldungeon.items.scrolls.ScrollOfTeleportation.teleportToLocation;
...
public class ScrollOfDebug extends Scroll {
    ...
    private enum Command {
        ...
        BROWSER(null, "[group|mod:<modId>]", "Opens the Debug Item Browser."),
        // generation commands.
        GIVE(Item.class,
    ...
    private static Map<String, String> getMacros() {
        if (macros == null) try {
            if (!FileUtils.fileExists(MACRO_FILE)) {
                macros = new HashMap<>();
                return macros;
            }
    ...
    /** token-tree autocomplete used by ScrollOfDebug inline ghost text. **/
    private static final class CommandTreeSuggestionEngine implements InlineSuggestionEngine {
        private final PackageTrie trie;
        private final HashMap<Class<?>, List<String>> classNamesByType = new HashMap<>();
        private final ArrayList<String> commandNames = new ArrayList<>();

        private CommandTreeSuggestionEngine(PackageTrie trie) {
            this.trie = trie;
            for (Command command : Command.values()) commandNames.add(command.toString());
            commandNames.sort(String.CASE_INSENSITIVE_ORDER);
        }

        @Override
        public String suggestSuffix(String fullText, int cursorPosition) {
            if (fullText == null || cursorPosition < 0 || cursorPosition > fullText.length()) return "";

            String beforeCursor = fullText.substring(0, cursorPosition);
            int tokenStart = beforeCursor.lastIndexOf(' ') + 1;
            String currentPrefix = beforeCursor.substring(tokenStart);
            String completeChunk = tokenStart > 0 ? beforeCursor.substring(0, tokenStart) : "";
            String[] completedTokens = splitTokens(completeChunk);
            int tokenIndex = completedTokens.length;

            String candidate = bestCandidate(candidatesFor(tokenIndex, completedTokens, currentPrefix), currentPrefix);
            if (candidate == null) return "";

            int sharedLength = Math.min(currentPrefix.length(), candidate.length());
            String suffix = candidate.substring(sharedLength);

            if (shouldAppendSpace(tokenIndex, completedTokens, candidate)
                    && (cursorPosition == fullText.length() || fullText.charAt(cursorPosition) != ' ')) {
                suffix += " ";
            }
            return suffix;
        }
        ...
        private Collection<String> candidatesFor(int tokenIndex, String[] completedTokens, String prefix) {
            TreeSet<String> candidates = new TreeSet<>(String.CASE_INSENSITIVE_ORDER);
            if (tokenIndex == 0) {
                candidates.addAll(commandNames);
                candidates.addAll(getMacros().keySet());
                if (prefix.startsWith(Variable.MARKER)) {
                    candidates.add(Variable.MARKER);
                    candidates.addAll(Variable.activeNames());
                }
                if (prefix.startsWith("!") && !lastCommand.isEmpty()) candidates.add("!!");
                return candidates;
            }

            String first = completedTokens[0];
            if (first.startsWith(Variable.MARKER)) {
                candidates.add("inv");
                candidates.add("cell");
                candidates.add("i");
                candidates.add("c");
            ...
            if (command.paramClass != null) {
                candidates.addAll(classCandidates(command.paramClass));
                candidates.addAll(Variable.activeNames());
            }
            if (command == Command.HELP) {
                ...
                candidates.addAll(getMacros().keySet());
            } else if (command == Command.WARP || command == Command.GOTO) {
                candidates.addAll(Variable.activeNames());
            }
            if (command == Command.USE || command == Command.INSPECT) {
                candidates.addAll(commandNames);
            }
        ...
        private List<String> classCandidates(Class<?> parent) {
            if (parent == null || trie == null) return Collections.emptyList();
            List<String> cached = classNamesByType.get(parent);
            if (cached != null) return cached;

            TreeSet<String> classNames = new TreeSet<>(String.CASE_INSENSITIVE_ORDER);
            for (Class<?> cls : trie.getAllClasses()) {
                if (parent.isAssignableFrom(cls) && canInstantiate(cls)) {
                    classNames.add(cls.getSimpleName());
                }
            }
            cached = new ArrayList<>(classNames);
            classNamesByType.put(parent, cached);
            return cached;
        }

        private String bestCandidate(Collection<String> candidates, String prefix) {
            if (candidates == null || candidates.isEmpty()) return null;
            String normalizedPrefix = prefix == null ? "" : prefix.toLowerCase(Locale.ROOT);
            for (String candidate : candidates) {
                if (candidate != null && candidate.toLowerCase(Locale.ROOT).startsWith(normalizedPrefix)) {
                    return candidate;
                }
            }
            return null;
        ...
        private boolean shouldAppendSpace(int tokenIndex, String[] completedTokens, String candidate) {
            if (candidate == null || candidate.isEmpty()) return false;
            if (tokenIndex == 0) {
                if (Variable.MARKER.equals(candidate) || "!!".equals(candidate)) return false;
                return Command.get(candidate) != null
                        || getMacros().containsKey(candidate)
                        || candidate.startsWith(Variable.MARKER);
            }
            return tokenIndex == 1
                    && completedTokens.length > 0
                    && completedTokens[0].startsWith(Variable.MARKER)
                    && ("inv".equalsIgnoreCase(candidate)
                    || "cell".equalsIgnoreCase(candidate)
                    || "i".equalsIgnoreCase(candidate)
                    || "c".equalsIgnoreCase(candidate));
        }
    }
    ...
    Class<? extends Window> readWindowClass() {
        return WndTextInput.class;
    }
    ...
    public void openCommandConsole() {
        GameScene.show(createCommandWindow());
    }

    Window createCommandWindow() {
        final InlineSuggestionEngine suggestionEngine = new CommandTreeSuggestionEngine(trie);
        return new WndTextInput("Enter Command:", null, "", 100, false,
                "Execute", "Cancel") {
            private String currentSuggestion = "";

            {
                textBox.setAlignment(Align.left);
                // muted gray ghost text: keep alpha below 0.5 so it stays secondary to typed input.
                textBox.setInlineSuggestionStyle(0x9A9A9A, 0.42f);
                updateSuggestion();
            }

            private void updateSuggestion() {
                currentSuggestion = suggestionEngine.suggestSuffix(textBox.getText(), textBox.getCursorPosition());
                textBox.setInlineSuggestion(currentSuggestion);
            }

            private boolean acceptSuggestion() {
                if (textBox.getCursorPosition() != textBox.getText().length()) return false;
                if (currentSuggestion == null || currentSuggestion.isEmpty()) return false;
                boolean accepted = textBox.acceptInlineSuggestion();
                if (accepted) updateSuggestion();
                return accepted;
            }
            ...
            @Override
            protected boolean onTabPressed(String text) {
                updateSuggestion();
                return acceptSuggestion();
            }

            @Override
            protected boolean onRightPressed(String text) {
                updateSuggestion();
                return acceptSuggestion();
            }
    ...
    static String parseBrowserInitialMenuKey(String[] input) {
    ...
    public boolean interpret(String... input) {
        ...
        else if (command == Command.BROWSER) {
            String initialMenuKey = parseBrowserInitialMenuKey(input);
            try {
                GameScene.show(new DebugItemBrowserWindow(this, initialMenuKey));
            } catch (Exception e) {
                reportException("Failed to open Debug Item Browser", e);
            }
            return false;
        }
    ...
    private static final String CHANGELOG
        = ""
        +"_2.3.0_:"
            +"\n_-_ Added inline ghost-text autocomplete for command input."
            +"\n_-_ Suggestions now update as you type and can be accepted with _Tab_ or _Right Arrow_."
            +"\n_-_ Added command-tree suggestion engine with support for commands, macros, classes, and active variables."
            +"\n_-_ Added text input hooks for suggestion rendering/acceptance and improved cursor-aware acceptance behavior."
            +"\n_-_ Package/class suggestion helpers were expanded to improve class completion coverage."
            +"\n"
        +"_2.2.0_:"
